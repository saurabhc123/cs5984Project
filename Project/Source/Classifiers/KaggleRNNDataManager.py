...
class KaggleRNNDataManager(object):
    def __init__(self):
    ...
    def __init__(self, trainingDataFileName, sess, fc7, word_vec):
        ...
        normalized_kaggle_instance = KaggleRNNDataSet(clean_kaggle_dataset)
        ...
        self.train = KaggleRNNDataSet(train[:,:-1], train[:,-1])
        ...
        self.test = KaggleRNNDataSet(test[:,:-1], test[:,-1])
        self.validation = KaggleRNNDataSet(validation[:,:-1], validation[:,-1])
    ...
    def read_data_from_csv(self, trainingDataFileName):
        with open(trainingDataFileName, 'rt') as csvfile:
            ...
            for row in reader:
                self.data.append(KaggleRNNSample(row, self.sess, self.fc7, self.word_vec))
        print(len(self.data))
